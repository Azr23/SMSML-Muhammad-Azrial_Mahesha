...
def load_dataset() -> pd.DataFrame:
    if not DATA_PATH.exists():
        raise FileNotFoundError(
            f"Dataset tidak ditemukan di {DATA_PATH}. Salin hasil preprocessing ke folder Membangun_model terlebih dahulu."
        )
...
def save_artifacts(model: Pipeline, x_test: pd.DataFrame, y_test: pd.Series) -> dict:
    ...
    preview.to_csv(ARTIFACT_DIR / "prediction_preview.csv", index=False)
...
def main() -> None:
    ...
    mlflow.set_experiment("phishing_email_detection_tuning")
    ...
    param_grid = {
        "preprocessor__sender_text__ngram_range": [(1, 1), (1, 2)],
        "preprocessor__sender_text__min_df": [1, 2],
        "preprocessor__subject_text__ngram_range": [(1, 1), (1, 2)],
        "preprocessor__subject_text__min_df": [1, 2],
    ...
    with mlflow.start_run(run_name="tuning_manual_logging"):
        ...
        metrics = save_artifacts(best_model, x_test, y_test)

        mlflow.log_param("dataset_path", str(DATA_PATH.name))
        mlflow.log_param("train_rows", int(x_train.shape[0]))
        mlflow.log_param("test_rows", int(x_test.shape[0]))
        mlflow.log_param("best_cv_f1", float(search.best_score_))
